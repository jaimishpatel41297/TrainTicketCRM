        ...
        try
        {
            ...
            if (ddlselectsqd.SelectedValue != "0")
            {
                if (ddlttname.SelectedValue == "0")
                    qry = "select distinct ta.*,tt.Name as ttname from TAMaster ta,TTMaster tt where ta.TTId=tt.Id and ta.TTId IN (select Id from ttmaster where SquadHeadId=" + ddlselectsqd.SelectedValue + " or Id=" + ddlselectsqd.SelectedValue + ");";
                else
                    qry = "select distinct ta.*,tt.Name as ttname from TAMaster ta,TTMaster tt where ta.TTId=tt.Id and ta.TTId IN (" + ddlttname.SelectedValue + ");";
            }
