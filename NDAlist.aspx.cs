...
public partial class NDAlist : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (ddlselectsqd.SelectedValue != "0")
            {
                if (ddlttname.SelectedValue == "0")
                    qry = "select distinct nm.*,tt.Name as ttname from NDAMaster nm,TTMaster tt where nm.TTId=tt.Id and nm.TTId IN (select Id from ttmaster where SquadHeadId=" + ddlselectsqd.SelectedValue + " or Id=" + ddlselectsqd.SelectedValue + ");";
                else
                    qry = "select distinct nm.*,tt.Name as ttname from NDAMaster nm,TTMaster tt where nm.TTId=tt.Id and nm.TTId IN (" + ddlttname.SelectedValue + ");";
            }
